...
// limitations under the License.
  ...
  extension Clients {
    protocol CmekConfigServiceStub {
    ...
    class CmekConfigServiceTransport: CmekConfigServiceStub {
        ...
        let path = try { () throws -> Swift.String in
          guard let pathVariable0 = request.config.map({ $0.name }), !pathVariable0.isEmpty else {
            throw GoogleCloudGax.RequestError.binding(
              "'request.config.name' is not set or is empty")
          }
      ...
      public func getCmekConfig(
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDiscoveryengineV1.CmekConfig.self, from: data)
      }
      ...
      public func listCmekConfigs(
        ...
        let path = try { () throws -> Swift.String in
          ...
          return "/v1/\(pathVariable0)/cmekConfigs"
        }()
        let query = [
          URLQueryItem(name: "$alt", value: "json;enum-encoding=int")
        ]
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDiscoveryengineV1.ListCmekConfigsResponse.self, from: data)
      }
