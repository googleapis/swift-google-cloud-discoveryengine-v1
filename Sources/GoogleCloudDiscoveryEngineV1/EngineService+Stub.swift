...
// limitations under the License.
  ...
  extension Clients {
    ...
    class EngineServiceTransport: EngineServiceStub {
      ...
      public func createEngine(
        ...
        let path = try { () throws -> Swift.String in
          ...
          return "/v1/\(pathVariable0)/engines"
        }()
        var query = [
      ...
      public func updateEngine(
        ...
        let path = try { () throws -> Swift.String in
          guard let pathVariable0 = request.engine.map({ $0.name }), !pathVariable0.isEmpty else {
            throw GoogleCloudGax.RequestError.binding(
              "'request.engine.name' is not set or is empty")
          }
          return "/v1/\(pathVariable0)"
        }()
        var query = [
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDiscoveryEngineV1.Engine.self, from: data)
      }
      ...
      public func listEngines(
        ...
        let (data, _) = try await self.inner.rpc(for: req).get()
        return try GoogleCloudWkt._ProtoJSONDecoder().decode(
          GoogleCloudDiscoveryEngineV1.ListEnginesResponse.self, from: data)
      }
